...
# this is Scale-Aware RFE Model include class TridentBlock、RFEM、C3RFEM three models
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolo.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        from utils.experimental import attempt_download, attempt_load  # scoped to avoid circular import
...
class AutoShape(nn.Module):
    ...
    def _apply(self, fn):
        ...
        from utils.yolo import Detect, Segment
        if self.pt:
            m = self.model.model.model[-1] if self.dmb else self.model.model[-1]  # Detect()
