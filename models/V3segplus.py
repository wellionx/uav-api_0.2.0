...
from models.CARAFE import *
from models.mixnetL import *
...
class EncoderDecoder(nn.Module):
    def __init__(self):
        ...
        self.carafe_down = CARAFE_downsampling(64, 64)
        self.carafe_up = CARAFE_upsampling(64,64)
        self.carafe_up2 = CARAFE_upsampling(264,64)
    ...
    def forward(self,x):
        dic = self.encoder(x)
        cmap = dic['one_8']
        one_8 = self.conv3(cmap) #convert channel dimension 56 to 64
        one_16 = dic['one_16']
        one_16 = self.conv4(one_16) #convert channel dimension 160 to 64
        one_32 = dic['one_32']

        counting_map = self.carafe_up2(one_32)
        if counting_map.size != one_16.size():
            counting_map = TF.resize(counting_map, size=one_16.size()[2:],antialias=True)
        counting_map = self.carafe_up(counting_map)
        if counting_map.size != one_8.size():
            counting_map = TF.resize(counting_map, size=one_8.size()[2:],antialias=True)

        output = self.aspp(one_32)
        # print(output.size())
        output = self.carafe_up(output)
        if output.size != one_16.size():
            output = TF.resize(output, size=one_16.size()[2:],antialias=True)
        # print(output.size())
        # print(one_16.size())
        output += one_16
        output = self.carafe_up(output)
        if output.size != one_8.size():
            output = TF.resize(output, size=one_8.size()[2:],antialias=True)
        # print(output.size())
        # print(one_8.size())
        output += one_8
        output = F.relu(self.bn(self.conv2(output)),inplace=True)
        dic = {"counting branch": counting_map, "segmentation branch": output}
...
class V3segplus(nn.Module):
    def __init__(self, input_size=64, output_stride=8):
        super(V3segplus, self).__init__()
...
if __name__ == '__main__':
    net=V3segplus().cuda()
    net.eval()
    x_image = torch.randn(1, 3, 456,688).cuda()
    dic = net(x_image)
    C = dic["C"]
    R = dic["R"]
    s = dic["segmentation"]
